...
public class FindUserRepository implements FindUserPort {
    ...
    @Override
    public Page<User> search(Long id, String username, String email, int page, int size) {
    ...
    @Override
    public List<User> findAllByScopeId(Long scopeId) {
    ...
    public User findByUsernameOrEmail(String username, String email) throws NotFoundException {
        ...
        UserJpa userJpa = optional.orElseThrow(() -> new NotFoundException("user not found by username: " + username + ", or email: " + email));
        return UserMapper.parse(userJpa);
    }
